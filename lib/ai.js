 ...
 */
async function prepareCampaignMessage(pitch, linkHint = '') {
...
- Placeholders {prénom}, {nom}, {phone}, {name} : les garder exactement tels quels
...
async function spinMessage(original, { prenom = '', index = 0, previousSpins = [] } = {}) {
  ...
  const avoid = previousSpins.length
    ...
    : '';
  const nameNote = prenom ? `\nGarde {prénom} si présent (ex. prénom réel : ${prenom}).` : '';
...
- Placeholders {prénom}, {nom}, {phone}, {name} inchangés
- Pas d'URL ajoutée
- Une seule variante, sans guillemets ni explication${nameNote}${avoid}`;
